...
| S3 | rotation d'âge vs rien vs biais scalaire | ph.11 : contrôle θ_âge=0 (HoPE), NON NÉGOCIABLE | **NON-RÉSULTAT 08-04 sur `rule`** (8 cellules finies) : acc 1,0 plafond partout, marges agezero ≥ tous les bras rotés — l'env n'a AUCUNE charge temporelle, la rotation n'y a rien à coder (objection user 08-04). Verdict final = zzs115/116 (ph.12, `age-log` × {fifo, attn-ema} sur env `life`, A/B exacts vs zzs101/103) | la rotation doit BATTRE θ_âge=0 sur la citation, sinon âge = biais scalaire de récence |
