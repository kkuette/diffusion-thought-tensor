...
| curriculum 2 phases à 97M (v350_curr_p1 local 3090 + v350_curr_p2 job 116) | le jumeau de COÛT du 350M (voie C, ÷3) : phase 1 batchée B=4 chunks fixes sans stack (recette v2b_mix, teacher ON, divmix, 1500 steps) puis phase 2 continued full-stack B=1 var_chunk (stack du 110, schedule v3_reach, 800 steps) — retombe-t-on sur les mécanismes du from-scratch joint ? | 🟡 phase 1 FINIE 2026-07-14 (1500 steps, ~3h, zéro NaN) : GAP positif 14/14 @1500 (codeparrot +1.39 carried 5.877, web +0.94..+1.09, GAP-par-profondeur plat d2-d8) ; phase 2 full-stack lancée ; VERT si carried ancres ±0.15 de 110 @2000 ET label-cue négatif aux probes | comparaison à tokens non appariés (chunks fixes 512 vs var 128-512 ⇒ phase 1 voit ~1.6× tokens/step) — juger mécanismes + carried, pas la loss à step égal ; transitoire fixe→var_chunk toléré ~200 steps |
| 350M validated run | the funded/self-funded scale point | pending full validation arc | plan in passation §2bis |

## Arc 6 — recentrage mémoire v2 : du RTI-comme-fin à la persistance à travers RESET

Le claim change de nature (SPEC_MEMOIRE_V2, éd. lissée 08-03) : conditionnement
+ rappel exact à travers des resets de fenêtre bornée, appariés en forwards
totaux. Cet arc est la chaîne de preuves qui a forcé chaque choix de
l'architecture consolidée ; les décisions encore ouvertes = registre S1-S17 de
la spec (une décision, un test, une règle).

| Test | Question | Verdict | Not established / caveat |
|---|---|---|---|
| phase 1 350M — 10B tokens (FINDINGS 2026-07-2x) | le pretrain SIF passe-t-il à 350M ? | ✅ 10B finis (pod 8×A100, incident NaN survécu), repass SIF stop@2500 POSITIF, ckpt sur HF | santé GAP, pas une preuve de rappel |
| SFT recall 350M : stair / table v1 / table2 (FINDINGS 2026-07-30) | le read fast-weight peut-il apprendre la citation ? | ❌ trois recettes mortes — dilemme fermé : codes discriminants inatteignables (v1) vs atteignables indiscriminables (v2) ⇒ le mur est la CLASSE DE FONCTIONS du read (une fonction, jamais un contenu citable) | a lancé le labo jouet + la voie copy-head ; GRPO recall en pause @17 |
| labo jouet read×banque ph.1-9 (commits f5aabeb→a897be6) | par où passe une citation exacte ? | ✅ chaîne retrieve-then-inject prouvée : ph.7 injection-oracle 0,708 en code vs TOUT readout appris mort (MoS incl.) — la citation passe par les pseudo-tokens NATIFS ; ph.8 retriever appris PARFAIT (recall@1 1,000, W_q+CE multi-positifs) et copie 0,66 si ordre d'injection = score vs 0,21 aléatoire ⇒ le circuit de copie est câblé au LAYOUT ; ph.9 états post-norm citent 0,434 from-scratch | short/word 0,000 même vérité injectée (prior LM) : le jouet est aveugle sur ces strates ; from-scratch d512 |
| port RTI 350M + diagnostic @800 (FINDINGS 2026-07-31) | le RTI porté cite-t-il au 350M ? | 🔶 le RTI est CORRECT, le mur = la COPIE du backbone (contrôle décisif : mêmes erreurs en contexte plat natif — induction heads réfutées) ; copy-head livrée (micro-train CE valeur 0,201 ON vs 2,212 OFF) | template se copie, la valeur non, AVANT copy-head |
| KT3 « biais chiffres » (FINDINGS 2026-08-01) | la sélection SIF déteste-t-elle les chiffres ? | ❌ prédiction RENVERSÉE : numeric inclusion 1,000 ; le vrai bug = budget k FIXE vs longueur de tour (code T=35 : 0,000) ; correctifs livrés (priorité de span, re-mesure 1,000 × 4 strates) | le fallback SIF pur garde un statut d'oracle |
| sonde localisation du budget (FINDINGS 2026-08-01) | où meurt l'information du write ? | ✅ le goulot = le POOLING appris 1-tête (rétention 1,00→0,47 ; moyenne uniforme 0,675) ; largeur gratuite, r second ordre ⇒ cadran désigné = m lignes par write | tap mi-stack restait à sonder |
| run copy 350M (FINDINGS 2026-08-03) | la chaîne complète cite-t-elle au 350M ? | ✅ PREMIER grade non nul : 0,28 vs 0,00 ablaté, p_copy val-only (0,566/0,005), hit 0,27 / miss 0,00 — retrieve-then-inject-then-copy FERMÉE | grade plafonné par la sélection, pas par la copie |
| KT1 surface vs compaction (FINDINGS 2026-08-03) | la banque bat-elle le résumé textuel ? | 🔶 OUI au rappel (0,343 vs 0,227, Δ apparié +0,115 CI [+0,089,+0,142]) — NON au nll de valeur (+0,223) : le préfixe+copy-head est un POINTEUR DISCRET ⇒ règle de protocole : Δnll ne juge plus la citation ; mur localisé = SÉLECTION (0,717 hit / 0,000 miss, r@1 0,480 multi-vies) | vs une compaction déflatée (retrieval gratuit) ; fenêtre bornée seulement |
| KT2 banque vs CoT explicite (FINDINGS 2026-08-03) | le précédent Coconut nous tue-t-il ? | ⚪ NON CONCLUANT : bras think jamais entraîné (gap de layout — leçon : train/éval isomorphes obligatoire) | re-run avec bras `<think>` entraîné = S11 |
| grille jouet ph.10 : 3 reads × rot × tap × m (FINDINGS 2026-08-03) | quelle lecture porte le gist ? | ✅ l'ATTENTION gagne : 2AFC 1,000 dès m=4 (shuf SOUS le hasard = spécifique au contenu), kv_append ≈ inject_entry ; le fast-weight PERD les deux régimes (multiplicatif diverge dès m>1 — Jacobiens, clip inopérant, fix additif 9215ddb bit-exact ; additif s'effondre avec m 0,97→0,65) ; m = cadran (citation croît m4→m8) ; age-rot sans gain ; citation 0,000 partout (lignes = états cachés, principe de séparation confirmé des 2 ailes) | tap indécidable à L6 ; max_mem=8 |
| carré factoriel projections × softmax ± bank-q (FINDINGS 2026-08-03 soir) | quelle forme d'attention-read ? | 🟡 36/48 — **S1 TRANCHÉ : dual_heads ÉLIMINÉ** (Δ nul, coût double, perd le budget m) ; **kvproj ADOPTÉ** (Δcit +0,209 ±0,115 apparié n=12, t=6,3, croît avec m : m8 2,09 vs 1,79, meilleure cellule rot-on m8 2,34) ; bias softmax appris ≈ 0 (la compétition de masse n'existait pas) ; age-rot DFT ~neutre dans kvproj (le design log/appris = ph.11) ; **S2 bank-q INVALIDÉ** : fuite non-causale (lanes voient le futur teacher-forcé, K/V des couches suivantes contaminés) — citation +1,45 factice + conditionnement effondré 2AFC 0,89→0,52 en m ⇒ bank-q au read RETIRÉ, contextualisation banque-lit-banque = étage WRITE | verdict à max_mem=8, KT8 dira s'il tient ; le carré est CLOS (S1 tranché, S2 invalidé — les 5 bq restantes documentent la fuite, rien de plus) |
| veille rotations-métadonnées (agent Opus, fiche ref-rotations-metadonnees-sota-2026) | l'encodage par rotation est-il soutenu par le SOTA ? | ✅ tag par rotation SOUTENU (SIREN-RoPE : l'additif nuit ; TS-RoPE : locuteur en sous-espaces) ; âge NON BORNÉ CONTREDIT (HoPE θ=0 gagne +22 % ; Base of RoPE : OOD structurel) ⇒ âge log-comprimé, 1 plan 0/π par canal, contrôle θ_âge=0 NON NÉGOCIABLE (S3) | trou de littérature : aucun tag catégoriel non ordonné publié en rotation pure — notre A/B (S5) est inédit |
| sonde spectre W_K' (FINDINGS 2026-08-03 nuit) | les clés banque se dockent-elles seules dans la bande LENTE du RoPE ? | ❌ corollaire §2.5 RÉFUTÉ : sur les 15 ckpts kvproj, l'énergie de W_K' par bande est indiscernable de l'uniforme (moitié lente 0,5000 ± 0,0005, ρ −0,035) — à fenêtre courte la contamination par R(t) ne coûte rien, rien ne pousse le modèle à s'en protéger ⇒ la prescription se tient PAR CONSTRUCTION | mesuré à max_mem=8 / fenêtre courte ; c'est la fenêtre LONGUE (350M) qui rend la contamination coûteuse — la mesure est scriptée, pas refaite |
| labo jouet ph.11 — 4 examens de rotation (harnais livré 2026-08-03 nuit, 22 cellules `zzr*`) | comment coder l'âge, le locuteur et l'index local sur les clés banque ? | 🟡 EN FILE : S3 âge {θ=0 contrôle HoPE / log / brut / biais scalaire} × m ; S4 OOD ×1/10/100 (+ augmentation d'échelle au train) ; S5 tag {rot 0/π par canal / additif / rien} sur env `prov` (deux faits même slot+attribut, clé oracle IDENTIQUE, ordre tiré au sort) avec r@1 et copie mesurés SÉPARÉMENT ; S17 index local {rot / add / rien} sur env `span` (longueurs MESURÉES 1..6). Rotations sur K' APRÈS W_K', plans DISJOINTS dockés sur les paires quasi statiques (garde de dérive qui REFUSE la construction si la bande déborde) | prédictions préenregistrées : log tient l'OOD et brut s'effondre ; l'écart rot−add croît avec la longueur du span ; la rotation peut GAGNER la sélection et CASSER la copie (câblée au layout, ph.8) — c'est pourquoi les deux sont mesurées à part. Envs `prov`/`span` : la question est DANS le seg gradé (sinon indécidable), donc leur layout n'est pas celui des ph.7-10 |
| spec lissée + registre S1-S17 (commits 86f442d→39a6a2f) | qu'est-ce qui reste ouvert ? | ⚪ consolidation : store unifié (lignes de d sans clé stockée, la clé = fonction kvproj), curseur Δn (provenance par pas de croissance, sélection sur delta + dédup), mem_dim = atomes/tour (jamais seq_len), 3 familles de rotation (âge log / canal 0/π / index local = opérateur successeur R_loc(1)), contrainte q partagé (plans sur dims quasi statiques, jamais de position de fenêtre côté banque) | tout ce qui est design-sans-mesure est DANS le registre S1-S17, rien n'est acté sans son test |
