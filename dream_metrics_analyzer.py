#!/usr/bin/env python3
"""
DREAM-Specific Metrics Analyzer
...
try:
    ...
    print("Warning: scipy not available, some correlation metrics will be simplified")
...
class ThoughtAnalyzer:
    ...
    def analyze_thought_evolution(self, 
        ...
        if self.previous_stack is not None:
            ...
            stack_diff = torch.norm(current_stack - self.previous_stack, dim=(1,2,3,4))
            ...
            for d in range(current_stack.shape[-1]):
                depth_diff = torch.norm(
                    current_stack[:,:,:,:,d] - self.previous_stack[:,:,:,:,d], 
                    dim=(1,2,3)
                ).mean().item()
    ...
    def analyze_thought_token_correlation(self, 
        ...
        for dim in range(thought_features.shape[1]):
            ...
            if len(set(accuracy)) > 1:  # Need variance in accuracy for correlation
                if SCIPY_AVAILABLE:
                    corr, p_value = stats.pearsonr(thought_dim, accuracy)
